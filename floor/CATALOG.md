...
**The R1-gate for this file: a quality director reads it cold and recognizes their own audit checklist.** Every check below is encoded and graded twice: against fixtures written for it, and against a synthetic OPO world authored without looking at it (`experiments/f-fixture/`). Encoding order: **SV-070 first** (the flagship — a reportable violation with no system check today), **then catalog order from SV-001.** Every check ships with passing AND failing fixtures before it counts as done.
